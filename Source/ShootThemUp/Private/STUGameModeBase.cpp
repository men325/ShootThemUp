...
void ASTUGameModeBase::CrateTeamsInfo() 
{
    if (!GetWorld()) return;
...
{
    const auto KillerPlyerState = KillerController ? Cast<ASTUPlayerState>(KillerController->PlayerState) : nullptr;
    const auto VictimPlyerState = VictimController ? Cast<ASTUPlayerState>(VictimController->PlayerState) : nullptr;
...
void ASTUGameModeBase::StopAllFire() 
{
    for (auto Pawn : TActorRange<APawn>(GetWorld()))
    {
        const auto WeapomComponent = STUUtils::GetSTUPlayerComponent<USTUWeaponComponent>(Pawn);
        if (!WeapomComponent) continue;

        WeapomComponent->StopFire();
        WeapomComponent->Zoom(false);
